...
exports.getSuggestedUsers = exports.searchUsers = exports.updateProfile = exports.getProfile = void 0;
    ...
    try {
        ...
        if (req.user && req.user.id !== profile.id) {
            const { data: friendship } = await supabase_1.supabaseAdmin
                ...
                .or(`and(requester_id.eq.${req.user.id},receiver_id.eq.${profile.id}),and(requester_id.eq.${profile.id},receiver_id.eq.${req.user.id})`)
                .single();
...
const getSuggestedUsers = async (req, res, next) => {
    try {
        ...
        const { data: friendIds } = await supabase_1.supabaseAdmin
            .from('friendships')
        ...
        const { data: suggestions, error } = await supabase_1.supabaseAdmin
            .from('profiles')
