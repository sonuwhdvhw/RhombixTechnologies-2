"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.viewStory = exports.createStory = exports.getStories = void 0;
const supabase_1 = require("../lib/supabase");
const errorHandler_1 = require("../middleware/errorHandler");
const getStories = async (req, res, next) => {
    try {
        const userId = req.user?.id;
        // Get active stories grouped by user (not expired)
        const { data: stories, error } = await supabase_1.supabaseAdmin
            .from('stories')
            .select(`
        *,
        profiles:user_id (id, username, full_name, avatar_url)
      `)
            .gt('expires_at', new Date().toISOString())
            .order('created_at', { ascending: false });
        if (error)
            throw (0, errorHandler_1.createError)(error.message, 400);
        // Group by user
        const grouped = new Map();
        stories?.forEach((story) => {
            const uid = story.user_id;
            if (!grouped.has(uid)) {
                grouped.set(uid, {
                    user: story.profiles,
                    stories: [],
                    hasUnviewed: false,
                });
            }
            const group = grouped.get(uid);
            group.stories.push(story);
        });
        // Check viewed status if authenticated
        if (userId) {
            const storyIds = stories?.map((s) => s.id) || [];
            if (storyIds.length > 0) {
                const { data: views } = await supabase_1.supabaseAdmin
                    .from('story_views')
                    .select('story_id')
                    .eq('viewer_id', userId)
                    .in('story_id', storyIds);
                const viewedIds = new Set(views?.map((v) => v.story_id));
                grouped.forEach((group) => {
                    group.hasUnviewed = group.stories.some((s) => !viewedIds.has(s.id));
                });
            }
        }
        res.json({ storyGroups: Array.from(grouped.values()) });
    }
    catch (err) {
        next(err);
    }
};
exports.getStories = getStories;
const createStory = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const { media_url, media_type = 'image', caption } = req.body;
        if (!media_url)
            throw (0, errorHandler_1.createError)('Media URL is required', 400);
        const { data: story, error } = await supabase_1.supabaseAdmin
            .from('stories')
            .insert({ user_id: userId, media_url, media_type, caption })
            .select(`
        *,
        profiles:user_id (id, username, full_name, avatar_url)
      `)
            .single();
        if (error)
            throw (0, errorHandler_1.createError)(error.message, 400);
        res.status(201).json({ story });
    }
    catch (err) {
        next(err);
    }
};
exports.createStory = createStory;
const viewStory = async (req, res, next) => {
    try {
        const { id } = req.params;
        const viewerId = req.user.id;
        await supabase_1.supabaseAdmin
            .from('story_views')
            .upsert({ story_id: id, viewer_id: viewerId }, { onConflict: 'story_id,viewer_id' });
        await supabase_1.supabaseAdmin
            .from('stories')
            .update({ views_count: supabase_1.supabaseAdmin.rpc('increment') })
            .eq('id', id);
        res.json({ message: 'Story viewed' });
    }
    catch (err) {
        next(err);
    }
};
exports.viewStory = viewStory;
//# sourceMappingURL=stories.js.map